			...
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					if blockType.storage != nil {
						...
						emptyHead.Flush()
						results, err = SelectFromBlocks(block.NewHeadBlock(emptyHead), blockType.blocks, matcher)
					}
...
func TestTraceIDIndexPath(t *testing.T) {
	...
	t.Run("Parquet block trace lookup", func(t *testing.T) {
		...
		results, err := SelectFromBlocks(block.NewHeadBlock(emptyHead), []block.Block{l1Block}, matcher)
		if err != nil {
