...
// MarshalBinary encodes a span event into binary format following the schema:
// u64be    span_id
// uvarint  name_len
// bytes    name
...
func (e *SpanEvent) MarshalBinary() ([]byte, error) {
	...
	estimatedSize := 8 + // span_id
		10 + len(e.Name) + // name
		10 + // timestamp
		10 + len(e.Attributes)*20 // attributes

	buf := make([]byte, 0, estimatedSize)

	// Parse span ID
	spanID, err := parseSpanID(e.SpanID)
	if err != nil {
		return nil, fmt.Errorf("invalid span ID: %w", err)
	}
	...
	// Write timestamp
	buf = appendSvarint(buf, e.Timestamp.UnixNano())
	...
	if len(e.Attributes) > 0 {
		...
		for _, key := range keys {
			buf = appendString(buf, key)
			buf = appendString(buf, e.Attributes[key])
		}
	}
...
// UnmarshalBinary decodes a span event from binary format
func (e *SpanEvent) UnmarshalBinary(data []byte) error {
	offset := 0

	// Read span ID
	if len(data) < 8 {
		return fmt.Errorf("data too short for span ID")
	}

	spanID := binary.BigEndian.Uint64(data[offset:])
	offset += 8
	e.SpanID = fmt.Sprintf("%016x", spanID)

	// Read name
	var err error
	e.Name, offset, err = readString(data, offset)
	if err != nil {
		return fmt.Errorf("failed to read name: %w", err)
	}
	...
	timestampNano, offset, err = readSvarint(data, offset)
	if err != nil {
		return fmt.Errorf("failed to read timestamp: %w", err)
	}
	e.Timestamp = unixNanoToTime(timestampNano)
	...
	// Read attributes
	e.Attributes = make(map[string]string, attributesCount)
