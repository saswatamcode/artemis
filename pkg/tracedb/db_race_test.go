...
func TestDB_QueryFlushRaceCondition(t *testing.T) {
		...
		go func(id int) {
			...
			for j := range 10 {
				...
				err := db.QueryWithLock(func(head *storage.ArrowStorage, blocks []block.Block) error {
					var queryErr error
					result, queryErr = query.SelectFromBlocks(block.NewHeadBlock(head, nil), blocks, matcher)
					return queryErr
				})
				if err != nil {
	...
	err = db.QueryWithLock(func(head *storage.ArrowStorage, blocks []block.Block) error {
		var queryErr error
		finalResult, queryErr = query.SelectFromBlocks(block.NewHeadBlock(head, nil), blocks, matcher)
		return queryErr
	})
	if err != nil {
